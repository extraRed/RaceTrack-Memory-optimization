...
{
	/*baseline:在0，16，32，48处分别有一个读/写端口*/
	for(int i=0;i<group_num;i++)
	{
		port_link[i].shift_num = 0;
		memset(port_link[i].port,0,sizeof(port_link[i].port));
		port_link[i].port[0] = RW_PORT;
		port_link[i].port[16] = RW_PORT;
		port_link[i].port[32] = RW_PORT;
		port_link[i].port[48] = RW_PORT;
	}	
} 
...
int main()
{
  	...
  	for(int i=0;i<8;i++)
  	{
		...
		init_param();
  		...
  		strcpy(output,"D:\\大三上\\计算机组织与体系结构\\大作业\\Red & Yuan\\Result1.0\\");
