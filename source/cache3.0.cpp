...
void RT_shift(int group_id, int domain_id, bool is_write)
{
	...
	/*静态端口选择策略*/
	//new_shift=domain_id - (select[domain_id] + this_one.shift_num);
...
{
  	/*优化：set横向排列，每个set横跨1/2/4/8个group*/
...
/*
void init_domain_portmap()		//定义端口管辖的domain 
{
	for(int i=0;i<16;i++)
		select[i]=7;
	for(int i=16;i<32;i++)
...
int main()
{
  	...
  	for(int i=0;i<8;i++)
  	{
		...
		init_param();
  		...
  		strcpy(output,"D:\\大三上\\计算机组织与体系结构\\大作业\\Red & Yuan\\Result3.0\\");
